...
std::string user_name_compare[18]={"chaileigang","chensijia","duanxintong","kangyuchangxia","lianlaoshi","lihongbin",
"liujianshan","lizhuoyang","luoyucui","shenhao","shijianhao","sunyuzhe","wangmeng",
"zhaiwenpeng","zhaozhuang","zhengwenjian","zhouyanhong","zhaoyue"};
 
std::string user_path_compare[18]={"./dataset/face_data/chaileigang/1.jpg","./dataset/face_data/chensijia/1.jpg","./dataset/face_data/duanxintong/1.jpg","./dataset/face_data/kangyuchangxia/1.jpg","./dataset/face_data/lianlaoshi/1.jpg","./dataset/face_data/lihongbin/1.jpg","./dataset/face_data/liujianshan/1.jpg","./dataset/face_data/lizhuoyang/1.jpg","./dataset/face_data/luoyucui/1.jpg","./dataset/face_data/shenhao/1.jpg","./dataset/face_data/shijiahao/1.jpg","./dataset/face_data/sunyuzhe/1.jpg","./dataset/face_data/wangmeng/1.jpg","./dataset/face_data/zhaiwenpeng/1.jpg","./dataset/face_data/zhaozhuang/1.jpg","./dataset/face_data/zhengwenjian/1.jpg","./dataset/face_data/zhouyanhong/1.jpg","./dataset/face_data/zhaoyue/1.jpg"};
...
void Compare::match(BaiduFaceApi *api)
{
    ...
    // res = api->match(token1.c_str(), 3, token2.c_str(), 3);
     ...
     //res = api->match("1.jpg", 2, "duizhao.jpg", 2);
...
//     std::string base64 = ImageBase64::file2base64("2.jpg");
 //    res = api->match("1.jpg", 2, base64.c_str(), 1);
  ...
  //   std::cout << "---res is:" << res << std::endl;
    ...
    //to do compare 1:1(img1 and img2)
    std::string out_buf1;
    int buf_len1 = ImageBuf::get_buf("1.jpg", out_buf1);
    std::string out_buf2;
    int buf_len2 = ImageBuf::get_buf("duizhao.jpg", out_buf2);

    const float *feature1 =nullptr;
    int dim_count1 = api->get_face_feature("1.jpg", 2, feature1);
    std::cout<<"dim count img1 is:"<<dim_count1<<std::endl;
    std::cout<<"feature img1 is:"<<feature1<<std::endl;

    for(int i = 0;i <dim_count1;i++)
    {
	std::cout<<"feature1"<<i<<"is"<<feature1[i]<<std::endl;
    }
    

    const float *feature2 =nullptr;
    int dim_count2 = api->get_face_feature("1.jpg", 2, feature2);
    std::cout<<"dim count img2 is:"<<dim_count2<<std::endl;
    std::cout<<"feature img2 is: "<<feature2<<std::endl;

    //std::string compare_result = api->compare_feature(*feature1,*feature2);
    //std::cout<<"compare result is:"<<compare_result<<std::endl;
...
void Compare::compare_feature(BaiduFaceApi *api)
{
    ...
    float score1 = api->compare_feature(v_feature1, v_feature2);
    std::cout << "compare score1 is:" << score1 << std::endl;

}

void Compare::compare_feature_from_dataset(BaiduFaceApi *api)
{   
    ...
    cv::Mat m1 = cv::imread("./dataset/face_data/zhaoyue/zhaoyue2.jpg");
    if (m1.empty())
    {
        std::cout << "mat1 is empty,please check img path" << std::endl;
    }
    ...
    for(int w = 0; w<18; w++)
    {
	image2_path = user_path_compare[w];
	std::cout<<image2_path<<std::endl;
        cv::Mat m2 = cv::imread(image2_path);
    	if (m2.empty())
    	{
        	std::cout << "mat2 is empty,please check img path" << std::endl;
    	}
    	...
    	float score1 = api->compare_feature(v_feature1, v_feature2);
    	std::cout << "compare score"<< w <<" is:" << score1 << std::endl;
	score_all[w] = score1;
	...
	if(max_score<score_all[k])
		...
		max_location = k;
		max_name = user_name_compare[max_location];
    ...
    std::cout<<"max location is:"<<max_location<<std::endl;
